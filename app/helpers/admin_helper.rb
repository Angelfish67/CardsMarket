module AdminHelper
  ...
  def admin_field_label(field)
    { "allowed_rarities" => "Erlaubte Seltenheitsstufen", "coins" => "Guthaben", "amount" => "Gutschrift", "reason" => "Begründung", "image" => "Hochgeladenes Bild", "image_url" => "Bild-URL", "name" => "Name", "description" => "Beschreibung", "rarity" => "Seltenheit", "base_value" => "Grundwert",
  ...
  def admin_value(value)
    return value.map { |item| item.to_s.humanize }.join(", ") if value.is_a?(Array)
